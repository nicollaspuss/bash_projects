...
echo "*****************************"
    ...
    echo
    read -p "1) Which PDB do you want to Clone? " pdb_name
    ...
    if echo "$PDBS" | grep -q -w "$pdb_name"; then
        ...
        if echo "$new_name" | grep -q -w "$PDBS"; then
            echo "The new PDB already exists."
            echo
            exit
        else 
            echo "The new name of the PDB does not exists. Processing."
            echo "Log: /tmp/clonepdb.txt"
            echo
            while true; do
            ...
            case "$user_response" in
                [yY])
                    ...
                    export ORACLE_HOME=/oracle/product/19.17.0.0.0
                    export PATH=PATH=/usr/local/bin:/bin:/usr/bin:/usr/local/sbin:/usr/sbin:/home/oracle/.local/bin:/home/oracle/bin:/oracle/product/19.17.0.0.0/bin:/oracle/product/19.17.0.0.0/OPatch:/usr/local/bin:/bin:/usr/bin:/usr/local/sbin:/usr/sbin:/home/oracle/.local/bin:/home/oracle/bin
                    sqlplus / as sysdba << EOFSQL | tee -a /tmp/clonepdb.txt
create pluggable database $new_name from $pdb_name;
alter pluggable database $new_name open;
show pdbs;
alter pluggable database $new_name save state instances=all;
exit;
EOFSQL
                    break
                    ;;
